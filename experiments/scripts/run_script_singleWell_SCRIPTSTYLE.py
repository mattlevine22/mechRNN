...
def main():
	...
	for state_init in my_state_inits:
		i += 1
		sim_model_params = {'state_names': ['u'], 'state_init':state_init, 'delta_t':delta_t, 'smaller_delta_t': min(delta_t, delta_t), 'ode_params':(a, b)}
		rnn_model_params = {'state_names': ['u'], 'state_init':state_init, 'delta_t':delta_t, 'smaller_delta_t': min(delta_t, delta_t), 'ode_params':(a, b)}
		all_dirs = []
	 ...
	 #      trivial_init=True)
		...
		for eps_badness in [0.05, 1, 10]:
			rnn_BAD_model_params = {'state_names': ['u'], 'state_init':state_init, 'delta_t':delta_t, 'smaller_delta_t': min(delta_t, delta_t), 'ode_params':(a, b*(1+eps_badness))}
