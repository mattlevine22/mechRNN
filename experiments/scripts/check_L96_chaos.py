...
def run_ode_model(model, tspan, sim_model_params, output_dir=".", plot_slow_indices=None, plot_fast_indices=None, do_plotting=True):
...
def make_plots(sim_model_params=SIM_MODEL_PARAMS, output_dir='.', K=4, J=4, F=10, eps_power=-7, hx=-0.8, delta_t=0.01, T=10, decoupled=False):
	...
	l96m = L96M(K=K, J=J, F=F, eps=eps, hx=hx)
...
def make_traj_plots(n_inits=2, sd_perturb=0.01, sim_model_params=SIM_MODEL_PARAMS, output_dir='.', K=4, J=4, F=50, eps=2**(-7), hx=-0.8, delta_t=0.01, T=5, decoupled=False, wspace=None, hspace=None):
	# delta_t = 0.01 # output interval for ODE simulation
	# T = 10 # Total length of ODE simulation
	# K = 4
	# J = 4
	...
	# write parameters to file
	settings_fname = os.path.join(output_dir,'check_chaos_settings.txt')
	with open(settings_fname, 'w') as f:
		json.dump(locals(), f, default=lambda x: getval(x) , indent=2)
	...
	Y = []
	sim_model_params['state_init'] = state_init
	y_clean = run_ode_model(sim_model, tspan, sim_model_params, output_dir=output_dir, plot_slow_indices=plot_slow_indices, plot_fast_indices=plot_fast_indices, do_plotting=False)
	Y.append(y_clean)

	for _ in range(n_inits-1):
		sim_model_params['state_init'] = state_init + sd_perturb*np.random.randn()
		y_clean_perturbed = run_ode_model(sim_model, tspan, sim_model_params, output_dir=output_dir, plot_slow_indices=plot_slow_indices, plot_fast_indices=plot_fast_indices, do_plotting=False)
		Y.append(y_clean_perturbed)

	fig, ax_list = plt.subplots(len(plot_slow_indices), 1, figsize=[11,11], sharex=True)
	for k in range(K):
		ax = ax_list[k]
		for y in Y:
...
if __name__ == '__main__':
	...
	make_traj_plots(n_inits=2, sd_perturb=0.5, sim_model_params=sim_model_params, output_dir=output_dir, F=F, eps=eps, T=T, decoupled=False)
