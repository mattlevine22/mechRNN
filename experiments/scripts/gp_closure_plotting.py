	...
	for n_subsample in [1000, 10000]:
		...
		for F in [50]:
			...
			for alpha in [100, 50, 25, 10, 1]:
				...
				fig.savefig(fname=os.path.join(basedir,'dt{dt}'.format(dt=dt),'1d_F{F}_GP_closure_n{n_subsample}_inferYbar{infer_Ybar}_comparison_alpha{alpha}.png'.format(infer_Ybar=infer_Ybar, n_subsample=n_subsample, alpha=alpha, F=F)), dpi=300)
				plt.close(fig)
