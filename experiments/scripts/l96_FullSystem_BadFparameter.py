...
parser.add_argument('--J', type=int, default=None, help='Default is to set J to value of K')
parser.add_argument('--F', type=int, default=10, help='constant forcing function. 10 is chaotic. 6.87 is quasi-periodic. 5 is periodic.')
parser.add_argument('--eps_scale', type=int, default=2**(-7), help='constant forcing function. 10 is chaotic. 6.87 is quasi-periodic. 5 is periodic.')
parser.add_argument('--slow_only', type=str2bool, default=False, help='')

FLAGS = parser.parse_args()

#python3 l96_sandbox.py --savedir ~/Downloads/l96_tests_v2/BDF_Init0 --delta_t 0.01 --t_train 10 --n_tests 1 --fix_seed True --t_test 10 --slow_only False
...
def main():
	...
	l96m_AVAIL = L96M(K=K, J=J, F=8, eps=FLAGS.eps_scale) # model used as a predictor
	...
	# set up state names
	state_names = ['X_'+ str(k+1) for k in range(l96m_TRUE.K)]
	for k in range(l96m_TRUE.K):
	  state_names += ['Y_' + str(j+1) + ',' + str(k+1) for j in range(l96m_TRUE.J)]
	...
	for state_init in my_state_inits:
		...
		if FLAGS.slow_only:
			stop_ind = l96m_TRUE.K
			y_clean_train = y_clean_train[:,:stop_ind]
			y_noisy_train = y_noisy_train[:,:stop_ind]
			y_clean_test_vec = y_clean_test_vec[:,:,:stop_ind]
			y_noisy_test_vec = y_noisy_test_vec[:,:,:stop_ind]
		...
		for n in range(FLAGS.n_experiments):
			for hidden_size in [50]:
				param_tuple = ()
				if FLAGS.slow_only:
					rnn_state_init = state_init[:l96m_TRUE.K]
					rnn_sim_model = l96m_TRUE.slow
				else:
					rnn_state_init = state_init
					rnn_sim_model = l96m_TRUE.full
			...
			for Fbad in np.random.permutation([4,5,6,7,8,9,9.9,10.0,10.1,11,12]):
				l96m_AVAIL_BAD = L96M(K=K, J=J, F=Fbad, eps=FLAGS.eps_scale)
				...
				if FLAGS.slow_only:
					rnn_BAD_sim_model = l96m_AVAIL_BAD.slow
				else:
					rnn_BAD_sim_model = l96m_AVAIL_BAD.full

				# FIRST run bad ODE model alone
				run_output_dir = output_dir + '/iter{0}'.format(n) + '/pureODE_Fbad{0}_clean'.format(Fbad)
				all_dirs.append(run_output_dir)
				if not os.path.exists(run_output_dir+'/fit_ode_TEST_{0}.png'.format(FLAGS.n_tests-1)):
					# torch.manual_seed(0)
					train_chaosRNN(forward_chaos_pureML,
						...
						run_output_dir, normz_info, rnn_BAD_sim_model,
						compute_kl=FLAGS.compute_kl,
						ode_only=True, plot_state_indices=plot_state_indices_SLOW)
				...
				for learn_flow in [False]:
					...
					# for learn_residuals in [True,False]:
						...
						if FLAGS.run_RNN:
							if not learn_flow:
								# mechRNN
								run_output_dir = output_dir + '/iter{0}'.format(n) + '/mechRNN_residual{1}_learnflow{2}_Fbad{3}_clean_hs{0}'.format(hidden_size, learn_residuals, learn_flow, Fbad)
								all_dirs.append(run_output_dir)
								...
								run_output_dir = output_dir + '/iter{0}'.format(n) + '/vanillaRNN_residual{1}_Fbad{2}_clean_hs{0}'.format(hidden_size, learn_residuals, Fbad)
								all_dirs.append(run_output_dir)
								if not os.path.exists(run_output_dir+'/rnn_fit_ode_TEST_{0}.png'.format(FLAGS.n_tests-1)):
									# torch.manual_seed(0)
									train_chaosRNN(forward_chaos_pureML,
