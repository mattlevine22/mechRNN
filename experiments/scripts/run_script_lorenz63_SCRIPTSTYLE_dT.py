	...
	for state_init in my_state_inits:
		...
		for delta_t in [0.1, 0.01, 0.05, 0.15, 0.2, 0.5]:
			...
			if FLAGS.n_train_points is not None:
				...
				print('Looping over dT with a fixed LENGTH of training time')
			...
			rnn_model_params = {'state_names': ['x','y','z'], 'state_init':state_init, 'delta_t':delta_t, 'smaller_delta_t': min(delta_t, delta_t), 'ode_params':(a, b, c), 'time_avg_norm':0.529, 'mxstep':FLAGS.mxstep}
			...
			for n in range(FLAGS.n_experiments):
				...
				for eps_badness in [0.05]:
					rnn_BAD_model_params = {'state_names': ['x','y','z'], 'state_init':state_init, 'delta_t':delta_t, 'smaller_delta_t': min(delta_t, delta_t), 'ode_params':(a, b*(1+eps_badness), c), 'time_avg_norm':0.529, 'mxstep':FLAGS.mxstep}
