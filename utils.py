...
def run_ode_model(model, tspan, sim_model_params, tau=50, noise_frac=0, output_dir="."):
  ...
  # collect normalization information
  normz_info_clean = {}
  normz_info_clean['Ymax'] = max(y_clean)
  normz_info_clean['Ymin'] = min(y_clean)
  normz_info_clean['Xmean'] = np.mean(x[:,1])
  normz_info_clean['Xsd'] = np.std(x[:,1])

  normz_info_noisy = {}
  normz_info_noisy['Ymax'] = max(y_noisy)
  normz_info_noisy['Ymin'] = min(y_noisy)
  normz_info_noisy['Xmean'] = np.mean(x[:,1])
  normz_info_noisy['Xsd'] = np.std(x[:,1])

  #normalize y (MIN/MAX [0,1])
  y_clean = (y_clean - normz_info_clean['Ymin'])/(normz_info_clean['Ymax'] - normz_info_clean['Ymin'])
  y_noisy = (y_noisy - normz_info_noisy['Ymin'])/(normz_info_noisy['Ymax'] - normz_info_noisy['Ymin'])

  #normalize x (MIN/MAX [0,1])
  x[:,1] = (x[:,1] - normz_info_noisy['Xmean']) / normz_info_noisy['Xsd']
  ...
  return y_clean, y_noisy, x, normz_info_clean, normz_info_noisy
...
def make_RNN_data(model, tspan, sim_model_params, noise_frac=0, output_dir="."):
  ...
  y_clean, y_noisy, x, normz_info_clean, normz_info_noisy  = run_ode_model(model, tspan, sim_model_params, noise_frac=noise_frac, output_dir=output_dir)
  ...
  input_data = torch.FloatTensor(x0.T)
  # input = torch.FloatTensor(np.concatenate((x0,y),axis=1).T)
  # output_data = torch.FloatTensor(y_noisy)

  return input_data, y_clean, y_noisy, normz_info_clean, normz_info_noisy
...
def train_RNN(forward,
  ...
  for i in range(n_epochs):
    ...
    loss_vec_train[i] = total_loss_train
    loss_vec_clean_train[i] = total_loss_clean_train
    ...
    for j in range(test_seq_length):
      ...
      hidden_state = hidden_state.detach()
    loss_vec_test[i] = total_loss_test
    loss_vec_clean_test[i] = total_loss_clean_test

    # print updates every 50 iterations or in 10% incrememnts
    if i % int( max(50, np.ceil(n_epochs/10)) ) == 0:
       print("Epoch: {}\nTraining Loss = {}\nTesting Loss = {}".format(
            i,
...
def compare_fits(my_dirs, output_dir="."):
  ...
  for d in my_dirs:
      ...
      ax1.plot(x, label=d.split("/")[-1])
      x = np.loadtxt(d+"/loss_vec_clean_test.txt")
      ax2.plot(x, label=d.split("/")[-1])
      x = np.loadtxt(d+"/loss_vec_test.txt")
      ax3.plot(x, label=d.split("/")[-1])
  ...
  fig.savefig(fname=output_dir+'/training_comparisons')
  plt.close(fig)
